...
def compare_genomes(genome_a, genome_b):
	...
	length_dict = {}
	for seq in a_seqs:
		length_dict[seq] = int(np.sum(a_df[a_df.iloc[:,0]==seq].iloc[:,2] - a_df[a_df.iloc[:,0]==seq].iloc[:,1]))
	for seq in b_seqs:
		length_dict[seq] = int(np.sum(b_df[b_df.iloc[:,0]==seq].iloc[:,2] - b_df[b_df.iloc[:,0]==seq].iloc[:,1]))
...
if __name__ == "__main__":
	...
	out_df.reset_index().drop('index', axis=1).to_csv('liftover_stats.tsv', sep='\t', index=False)